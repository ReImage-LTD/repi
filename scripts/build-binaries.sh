#!/usr/bin/env bash
#
# Build Repi binaries for all platforms locally.
...
#     repi-darwin-arm64.tar.gz
#     repi-darwin-x64.tar.gz
...
for platform in "${PLATFORMS[@]}"; do
    ...
    # standalone binary before Repi starts (see #7684).
    if [[ "$platform" == windows-* ]]; then
        bun build --compile --no-compile-autoload-bunfig --target="$bun_target" ./dist/bun/cli.js ./src/utils/image-resize-worker.ts --outfile "$OUTPUT_DIR/$platform/repi.exe"
    else
        bun build --compile --no-compile-autoload-bunfig --target="$bun_target" ./dist/bun/cli.js ./src/utils/image-resize-worker.ts --outfile "$OUTPUT_DIR/$platform/repi"
    fi
done
