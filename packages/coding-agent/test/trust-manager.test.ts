...
describe("ProjectTrustStore", () => {
	...
	it("detects trust-requiring project resources", () => {
		...
		try {
			mkdirSync(join(tempDir, ".repi", "agent"), { recursive: true });
			...
			writeFileSync(join(tempDir, ".repi", "settings.json"), "{}");
			expect(hasTrustRequiringProjectResources(tempDir)).toBe(true);
			rmSync(join(tempDir, ".repi", "settings.json"), { force: true });

			mkdirSync(join(cwd, ".repi"), { recursive: true });
			writeFileSync(join(cwd, ".repi", "settings.json"), "{}");
